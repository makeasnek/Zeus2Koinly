...
if __name__ == "__main__":
    ...
    for csv_type, known_path in known_paths.items():
        print(f'Processing CSV type: {csv_type} from {known_path}')
        with open(known_path, mode='r') as file:
            csvFile = csv.DictReader(file)
            # read in CSV file line by line
            for line in csvFile:
                if 'circular rebalance' in str(line).lower():
                    print(f'Skipping circular rebalance: {line}')
                    continue
                elif 'closed lightning channel' in str(line).lower():
                    print(f'Skipping LN channel close: {line}')
                    continue
                ...
                for key, value in line.items():
                    ...
                    if key in ignore_keys:
                        ...
                        if value.strip()=='':
                            #print(f'Skipping expired invoice: {line}')
                            continue
                        ...
                        print(f"Error: found unknown key {key}, exported CSV may not be complete! {line}")
                        print(f"{line}")
                        continue
                    else:
